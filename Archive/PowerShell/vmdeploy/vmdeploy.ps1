...
catch
{
	...
	$a = Get-Content $transpathrev
	$a > $transpathrev
	Send-MailMessage -From "automation@maritz.com" -To "$emailaddress" -Subject "Provisioning Complete For - $myVmname" -Body "Provisioning is complete.  Local OS build log can be found at [ C:\build\OSDeployLog.txt ] on $myVmname"
}